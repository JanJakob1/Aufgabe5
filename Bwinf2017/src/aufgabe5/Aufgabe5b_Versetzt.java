...
public class Aufgabe5b_Versetzt implements Runnable {

	public static Aufgabe5b_Versetzt Aufgabe;
	public Renderer_bVersetzt renderer;
	...
	int[][] pawn = new int[8][2];

	Aufgabe5b_Versetzt() {

		initComponents();
		renderer = new Renderer_bVersetzt();
		pawn[7][0] = 1;
		for (int i = 0; i < 3; i++) {
			pawn[i * 2 + 1][1] = 1;
		}
	...
	public void repaint(Graphics g) {
		...
		while (p <= 7) {
			...
			if (pawn[p][0] == 0) {
				g.drawImage(image2, Pawnposition_x + p * Playerheight, Pawnposition_y - pawn[p][1] * Playerheight,
						Playerheight, Playerheight, null);
			} else {
				j = p; // Freie Reihe, hier möchte der Turm also durchkommen und stellt sich deshalb
						// hier auf
			}
			p++;
	...
	public void run() {
		...
		while (o < 4) { // Bauern haben den Turm erst dann sicher gefangen, wenn sich jeder von ihnen
						// auf der vorletzten oder letzen Reihe des Sschachbretts befindet
	...
	public static void main(String[] args) {
		Aufgabe = new Aufgabe5b_Versetzt();
	}
